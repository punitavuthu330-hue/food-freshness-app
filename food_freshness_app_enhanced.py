"""Enhanced Streamlit app with authentication and database"""
...
import requests
from config import API_BASE_URL, FRESHNESS_COLORS, QUALITY_THRESHOLDS
from utils.freshness import get_freshness_status, calculate_average_freshness, calculate_average_quality, count_premium_ingredients
...
st.markdown("""
    ...
    .header-section { padding: 20px 0; border-bottom: 2px solid #e0e0e0; margin-bottom: 20px; }
    </style>
...
if 'quality_filter' not in st.session_state:
    st.session_state.quality_filter = 80
if 'cart' not in st.session_state:
    st.session_state.cart = []

# Sample data - restaurant menu with ingredient freshness (fallback if API unavailable)
MENU_DATA = {
    "Caesar Salad": {
        "price": 12.99,
        "ingredients": {
...
# ==================== SIDEBAR ====================
with st.sidebar:
    st.markdown("<div class='header-section'><h1>🥗 Fresh & Fair</h1></div>", unsafe_allow_html=True)
    ...
    # Filters
    st.header("🔍 Filters")
    st.session_state.quality_filter = st.slider("Minimum Quality Score", 0, 100, 80)
    
    freshness_priority = st.select_slider(
        "Show me ingredients...",
...
for dish, data in MENU_DATA.items():
    if all(ing['quality_score'] >= st.session_state.quality_filter for ing in data['ingredients'].values()):
        filtered_menu[dish] = data

if not filtered_menu:
    st.warning(f"No dishes meet your quality requirements (≥{st.session_state.quality_filter}). Showing all dishes...")
    filtered_menu = MENU_DATA

# Display dishes
for dish_name, dish_data in filtered_menu.items():
    with st.expander(f"🍽️ **{dish_name}** - ${dish_data['price']:.2f}", expanded=False):
        ...
        for idx, (ing_name, ing_info) in enumerate(dish_data['ingredients'].items()):
            with cols[idx]:
                status, emoji = get_freshness_status(ing_info['days_fresh'], ing_info['max_days'])
                css_class = f"freshness-{status}"

                html_content = f"""
                <div class='{css_class}'>
                    <h4>{ing_name}</h4>
                    <p><strong>Status:</strong> {emoji} {status.capitalize()}</p>
        ...
        col_add, col_qty = st.columns([3, 1])
        with col_qty:
            quantity = st.number_input("Qty", min_value=1, value=1, key=f"qty_{dish_name}")
        with col_add:
            if st.button("🛒 Add to Cart", key=f"add_{dish_name}"):
                st.session_state.cart.append({
                    "name": dish_name,
                    "price": dish_data['price'],
                    "quantity": quantity
                })
                st.success(f"Added {dish_name} to cart!")

# Footer
st.markdown("---")
col1, col2, col3 = st.columns(3)
with col1:
    st.button("📱 Place Order", use_container_width=True, key="place_order")
with col2:
    st.button("💬 Ask about origin", use_container_width=True, key="ask_origin")
with col3:
    st.button("📊 See nutrition info", use_container_width=True, key="nutrition")
